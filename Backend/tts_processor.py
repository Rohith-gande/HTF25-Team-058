...
"""
Text-to-Speech processing module using gTTS only (no pydub or ffmpeg required).
"""
import os
import uuid
from typing import List
...
class TTSProcessor:
    """Handles text-to-speech conversion and simple audio merging."""
    ...
    async def merge_audio_files(self, audio_files: List[str], output_path: str) -> str:
        """Merge multiple MP3 files into one (no ffmpeg required)."""
        try:
    ...
    async def convert_text_to_speech(self, text: str) -> str:
        """Convert text to a single speech file."""
        try:
            podcast_id = str(uuid.uuid4())
            final_audio_path = os.path.join(self.temp_dir, f"podcast_{podcast_id}.{self.audio_format}")
