...
const ProfileView = () => { // pass idToken if you implement auth later
  const [user, setUser] = useState(null);
  const [stats, setStats] = useState({ total: 0, duration: 0 });
  const [loading, setLoading] = useState(true);

  useEffect(() => {
  const idToken = localStorage.getItem("idToken"); // directly get token
  ...
  const fetchProfile = async () => {
    try {
      ...
      console.error(err);
      alert(err.response?.data?.detail || "Failed to fetch profile");
    } finally {
      setLoading(false);
    }
  };
